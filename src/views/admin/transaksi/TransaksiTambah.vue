...
const fnGetTotalTagihan = () => {
    let totalTagihan = dataKeranjang.value.reduce(function (acc, obj) { return acc + (obj.harga_beli_number * obj.jml); }, 0);
    return totalTagihan;
}
...
const onKeranjangUpdateBarang = async (values) => {
    ...
    if (parseInt(dataFormKeranjang.value.jml) > parseInt(tempKeranjangEdit.value.stok_tersedia)) {
...
const onPeriksaBayar = async (values) => {
    dataFormKonfirmasiBayar.value.uangBayar_number = dataFormKonfirmasiBayar.value.uangBayar.match(numberPattern).join('');
    let result = fnPeriksaPembayaran(dataFormKonfirmasiBayar.value.uangBayar_number);
    ...
    dataFormKonfirmasiBayar.value.uangKembalian = Fungsi.formatRupiah(result, "Rp. ");
}
        ...
        <div class="grid grid-cols-2">
                        ...
                        <table class="table w-full">
                            ...
                            <tbody>
                                ...
                                <tr v-for="d, index in data">
                                    ...
                                    <th>
                                        <button class="btn btn-warning btn-xs" @click="addToCart(d.id)"
                                            v-if="d.stok_tersedia > 0">Add to
                                            Keranjang</button>
                                        <button class="btn btn-danger btn-xs"
                                            @click="Toast.danger('Info', 'Stok Habis!')" v-else>Add
                                            to
                                            Keranjang</button>
            ...
            <div class="">
                <div>
                    ...
                    <h4>Total Tagihan : {{ Fungsi.formatRupiah(dataKeranjangInfo.totalTagihan, 'Rp. ') }}</h4>
                </div>
                <div>
    ...
    perPageDropdown: [10, 20, 50],
                        ...
                        <template #table-row="props">
                            ...
                            <span v-if="props.column.field == 'total'">
                                {{ Fungsi.formatRupiah((props.row.jml * props.row.harga_beli_number), 'Rp. ') }}
                            </span>
                            ...
                            <div class="space-y-4">
                                <div class="flex flex-col">
                                    <label for="name" class="text-sm font-medium text-gray-900 block mb-2">Jumlah
                                        Barang | <code>Max: {{ tempKeranjangEdit.stok_tersedia }}</code></label>
                    ...
                    <div class="py-2 lg:py-4 px-4">
                        <div class="flex flex-col">
                            <label for="name" class="text-sm font-medium text-gray-900 block mb-2">
                                Total Tagihan : {{ Fungsi.formatRupiah(dataKeranjangInfo.totalTagihan, 'Rp. ') }}

                            </label>

                        </div>
                        <div class="flex flex-col">
                            <label for="name" class="text-sm font-medium text-gray-900 block mb-2">Uang Bayar
                                <!-- {{ labelRupiah }}  -->
                            </label>
                            <Field v-model="dataFormKonfirmasiBayar.uangBayar" :rules="fnValidasi.validateData"
                                type="text" name="uangBayar" ref="uangBayar"
                                class="input input-bordered md:w-full max-w-2xl" required
                        ...
                        <div class="space-y-4">
                            <div class="flex flex-col">
                                <label for="name" class="text-sm font-medium text-gray-900 block mb-2">Status : {{
                                        dataFormKonfirmasiBayar.status
                                }} : {{ dataFormKonfirmasiBayar.uangKembalian
}}</label>
