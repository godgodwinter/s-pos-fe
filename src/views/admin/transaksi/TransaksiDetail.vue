...
const qrLink = ref(`${BASE_URL_FE}kodetrans/kodetrans/${kodetrans.value}`);
...
const getDataDetail = async () => {
    try {
        // const response = await Api.get(`admin/produk/${kodetrans.value}/kodetrans`);
...
const doPergiKeLink = () => {
    return router.push({
        name: "transaksi-detail-kodetrans", params: {
...
<template>
    ...
    <div>
        ...
        <div class="space-y-4">
            ...
            <div>
                <router-link :to="{
                    name: 'transaksi-detail-kodetrans', params: { kodetrans: kodetrans }
                }" target="_blank">
